...
- (id) reduceStart:(id)startValue
         withBlock:(id  _Nullable (^)(id _Nullable, id _Nullable))reduceBlock
{
...
+ (GPSequence<GPSequence *> *) zipSequences:(id<NSFastEnumeration>)zippedEnumeration
{
    ...
    if (!zippedEnumeration) {
        return nil;
    }
    
    NSMutableArray<GPSequence *> *result = [NSMutableArray new];
    GPSequence<id<NSFastEnumeration>>* tmp = GP_NEW_SEQ_WITH_TYPE(id<NSFastEnumeration>, zippedEnumeration);

    GPSequence<GPSEnumerator *> *sequences = [tmp map:^id _Nonnull(id<NSFastEnumeration>  _Nonnull item) {
